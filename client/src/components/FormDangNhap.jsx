import { ArrowRight, Eye, EyeOff, Loader, Lock, Mail, UserPlus } from 'lucide-react'
import React from 'react'
import { Link } from 'react-router-dom'

const FormDangNhap = ({email, setEmail, password, setPassword, showPassword, setShowPassword, handleSubmit, loading}) => {
  return (
    <div className="bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
        ...
        <p className="mt-8 text-center text-sm text-gray-400">
            Chưa có tài khoản ? { " " }
            <Link to="/dangky" className="font-medium text-emerald-400 hover:text-emerald-500"> 
            Đăng ký ngay <ArrowRight className='inline h-4 w-4' /></Link>
        </p>
    </div>
  )
}
