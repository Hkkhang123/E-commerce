    ...
    return (
        ...
        shadow-lg z-40 transition-all duration-300 border-b border-emerald-800'>
            <div className="container mx-auto px-4 py-3">
                ...
                <Link 
                    to={"/"} 
                    className='text-2xl font-bold text-emerald-400 items-center space-x-2 flex'>E-commerce</Link>

                <nav className='flex flex-wrap items-center gap-4'>
                    <Link 
                        to={"/"} 
                        className='text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out'>Trang chủ</Link>
                    {user && (
                        <Link to="/giohang" className='relative group'>
                            <ShoppingCart className='inline-block mr-1 group-hover: text-emerald-400' size={20} />
                            <span className='hidden sm:inline'>Giỏ hàng</span>
                            {cart.length > 0 && 
                                <span className='absolute left-2 -top-2 bg-emerald-500 text-white rounded-full px-2 
                    ...
                    {isAdmin && (
                        <Link className='bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1 
